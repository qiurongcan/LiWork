...
能力2-能力和技能使用状况：根据提供的这份职业信息，判断这个职业需要使用技能的程度：0:不需要；1:不太需要；2:一般需要；3:比较需要；4:非常需要
...
能力11-普通话能力：根据提供的这份职业信息，判断这份职业需要普通话的程度，其中，0:不需要；1:不太需要；2:一般需要；3:比较需要；4:非常需要
...
def getSkill():
    ...
    for i in range(0,len(origin_data.index),10):
        # 读取每一行的数据
        # data = origin_data[i]
        # if i < 1384:
        #     continue
